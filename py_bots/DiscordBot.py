...
from py_controls import EconomyManager
from py_controls import MilitaryManager
from py_controls import MemoryManager
...
def setupDiscord():
    ...
    !rss DU -> update given RSS-feed (eg. DU forum announcements)
    """))
    ...
    @register_command
    async def rss(msg, mobj):
        """
        Update given rss-feed, and post the message in in the chat
        Example: !rss DU
        """
        if msg == "":
            return await client.send_message(mobj.channel, "You didn't say anything!")
        if msg == "vc":
            vcStr = ""
            vcStr += (RSS_reader.getRSSupdates('vulture announcements', 'http://www.vulturecorporation.com/forum/m/20530240/op/rss/forum_id/3849083'))
            vcStr += (RSS_reader.getRSSupdates('vulture gaming', 'http://www.vulturecorporation.com/forum/m/20530240/op/rss/forum_id/3849082'))
            return await client.send_message(mobj.channel, vcStr)
        if msg == "du":
            duStr = (RSS_reader.getRSSupdates('dual universe announcements', 'https://board.dualthegame.com/index.php/rss/forums/1-du-forum-rss-feed/'))
            return await client.send_message(mobj.channel, duStr)
        if msg == "clear":
            RSS_reader.clearRSS()
            return await client.send_message(mobj.channel, "database cleared")
        return await client.send_message(mobj.channel, "unknown RSS-address")
    ###############################################################################
